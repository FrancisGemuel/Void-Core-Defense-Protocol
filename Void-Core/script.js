...
function spawnEnemy(waveNum) {
    const hp = 40 + waveNum * 18;
    const spd = 0.6 + waveNum * 0.04;
    const wp = getWaypoints();
...
function drawBase() {
    const cx = canvas.width * 0.93, cy = canvas.height * 0.65;
    ctx.save();
    ctx.beginPath();
                ...
                if (p.splash) {
                    for (let en of enemies) {
                        if (!en.dead && Math.hypot(en.x - p.target.x, en.y - p.target.y) < p.splashR) {
                            en.hp -= p.atk * 0.6;
                            if (en.hp <= 0) { en.dead = true; coins += en.reward; addParticles(en.x, en.y, en.color, 8); updateUI(); }
                        }
                    }
                } else {
                    p.target.hp -= p.atk;
                    if (p.target.hp <= 0) { p.target.dead = true; coins += p.target.reward; addParticles(p.target.x, p.target.y, p.target.color, 8); updateUI(); }
                }
